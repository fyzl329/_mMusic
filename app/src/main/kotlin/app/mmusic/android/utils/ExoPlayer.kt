...
class CatchingDataSourceFactory(
    ...
    private val fallbackFactory: DataSource.Factory? = null,
    private val fallbackResolver: ((DataSpec, Throwable) -> DataSpec?)? = null,
    private val onFallback: (() -> Unit)? = null
) : DataSource.Factory {
    inner class Source : DataSource {
        private var current: DataSource = parent.createDataSource()

        override fun addTransferListener(transferListener: TransferListener) =
            current.addTransferListener(transferListener)

        override fun open(dataSpec: DataSpec): Long = runCatching {
            current.open(dataSpec)
        }.getOrElse { ex ->
            val fallbackSpec = fallbackResolver?.invoke(dataSpec, ex)
            val fallback = fallbackFactory

            if (fallbackSpec != null && fallback != null) {
                runCatching { current.close() }
                current = fallback.createDataSource()
                onFallback?.invoke()
                return current.open(fallbackSpec)
            }
        ...
        override fun getResponseHeaders() = current.responseHeaders
...
fun DataSource.Factory.handleUnknownErrors(
    ...
    fallbackFactory: DataSource.Factory? = null,
    fallbackResolver: ((DataSpec, Throwable) -> DataSpec?)? = null,
